...
export default function DashboardShell({ children, user, profile }: DashboardShellProps) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      {/* Mobile Header */}
      <header className="fixed top-0 left-0 right-0 z-40 flex h-14 items-center justify-between border-b border-slate-200/60 bg-white/90 backdrop-blur-md px-4 lg:hidden">
        <button
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-72 border-r border-slate-200/60 bg-slate-50/80 backdrop-blur-sm
                    ...
                    {editingId === analysis.id ? (
                      <div className="flex w-full items-center px-3 py-2">
                        <input
                          ...
                          autoFocus
                          className="w-full rounded border border-teal-300 bg-white px-2 py-1 text-sm text-slate-700 outline-none focus:ring-2 focus:ring-teal-400"
                        />
                      </div>
                    ) : (
                      <>
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <p className="truncate font-medium text-slate-700 group-hover:text-teal-600">
                              {analysis.title}
                            </p>
