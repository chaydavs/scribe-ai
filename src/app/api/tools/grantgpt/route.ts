...
export async function POST(request: Request) {
  try {
    ...
    const { projectInfo, grantInfo, outputType } = await request.json()
    ...
    let userMessage = `Output Type: ${outputType || 'full'}\n\nProject Information:\n${projectInfo}`
    if (grantInfo) {
      userMessage += `\n\nGrant/Funder Information:\n${grantInfo}`
    }

    const response = await generateWithClaude(
      toolPrompts.grantgpt,
      userMessage
    )
    ...
    await serviceClient.from('credit_transactions').insert({
      ...
      type: 'usage',
      tool: TOOL_NAME,
      description: 'Grant content generation',
    })
    ...
    console.error('GrantGPT error:', error)
    return NextResponse.json(
      { error: 'Failed to generate grant content' },
      { status: 500 }
    )
  }
}
