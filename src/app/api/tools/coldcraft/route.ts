...
export async function POST(request: Request) {
  try {
    ...
    const { recipientInfo, senderInfo, purpose } = await request.json()

    if (!recipientInfo || !purpose) {
      return NextResponse.json(
        { error: 'Recipient info and purpose are required' },
        { status: 400 }
      )
    }

    let userMessage = `Recipient Information:\n${recipientInfo}\n\nPurpose:\n${purpose}`
    if (senderInfo) {
      userMessage += `\n\nSender Information:\n${senderInfo}`
    }

    const response = await generateWithClaude(
      toolPrompts.coldcraft,
      userMessage
    )
    ...
    await serviceClient.from('credit_transactions').insert({
      ...
      type: 'usage',
      tool: TOOL_NAME,
      description: 'Cold email generation',
    })
    ...
    console.error('ColdCraft error:', error)
    return NextResponse.json(
