...
export async function POST(request: Request) {
  try {
    ...
    const { data, briefType, audience } = await request.json()
    ...
    let userMessage = `Brief Type: ${briefType || 'summary'}\n\nData/Document:\n${data}`
    if (audience) {
      userMessage += `\n\nTarget Audience: ${audience}`
    }

    const response = await generateWithClaude(
      toolPrompts.databrief,
      userMessage
    )
    ...
    await serviceClient.from('credit_transactions').insert({
      ...
      type: 'usage',
      tool: TOOL_NAME,
      description: 'Data brief generation',
    })
    ...
    console.error('DataBrief error:', error)
    return NextResponse.json(
